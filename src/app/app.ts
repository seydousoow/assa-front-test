...
export class App implements OnInit {
  ...
  ngOnInit() {
    this.http.get('https://assa-auth.dicortex.com/v1/oidc/nonce', {responseType: 'text'}).subscribe({
      next: data => {
  ...
  logout() {
    this.http.post<void>('https://assa-auth.dicortex.com/v1/auth/logout', {nonce: localStorage.getItem('nonce') ?? ''}).subscribe();
  ...
  private initGoogleOauth(nonce: string): void {
    google.accounts.id.initialize({
      ...
      callback: ({credential}: CredentialResponse) => {
        this.http.post<IResponse>('https://assa-auth.dicortex.com/v1/oidc/google', {credential, nonce: this.nonce()}).subscribe({
          next: data => {
